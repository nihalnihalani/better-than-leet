...
import type { SystemDesignNodeType } from './CustomNodes';
...
export function SystemDesignPanel() {
  const selectedTopicId = useInterviewStore((s) => s.selectedTopicId);
  ...
  // Track which expected components have been placed
  const placedTypes = new Set<string>(diagramNodes.map((n) => n.type));
    ...
    <div className="h-full flex flex-col bg-card">
      ...
      <ScrollArea className="flex-1">
        <div className="p-4 space-y-6">
          ...
          <div>
            <h4 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-3">
              Components ({placedTypes.size}/{topic.expectedComponents.length})
            </h4>
            <div className="space-y-1.5">
              {topic.expectedComponents.map((comp) => {
                const isPlaced = placedTypes.has(comp);
                return (
                  <div
                    key={comp}
